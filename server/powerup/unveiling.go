...
// Activation is applied in the game layer (Player.HighlightIndices set to unknown-hidden indices). The effect
// lasts only for the current turn and is cleared when the turn ends (match, mismatch, or timeout).
// When Chaos is used, KnownIndices is cleared and HighlightIndices is reset for both players.
...
func (d *UnveilingPowerUp) Apply(board *game.Board, active *game.Player, opponent *game.Player, ctx *game.PowerUpContext) error {
	// No-op; highlight is activated in game.handleUsePowerUp (HighlightIndices).
	return nil
}
