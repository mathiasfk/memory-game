...
func TestRunExitsOnGameOver(t *testing.T) {
	...
	params := &config.AIParams{Name: "Mnemosyne", DelayMinMS: 10, DelayMaxMS: 20, UseBestMoveChance: 85, ArcanaRandomness: 0}
...
func TestPickPair_UsesHighlightWhenPresent(t *testing.T) {
	...
	for range 20 {
		first, _, _ := pickPair(memory, hidden, true, hiddenHighlighted, nil, nil, nil)
		if first != 2 && first != 4 {
	...
	first, _, _ := pickPair(memory, hidden, true, nil, nil, nil, nil)
	if first < 0 || first > 5 {
...
func TestPickSecondCard_UsesHighlightWhenPresent(t *testing.T) {
	...
	for range 20 {
		second, _ := pickSecondCard(memory, hidden, 3, true, hiddenHighlighted, nil, nil, nil, nil)
	...
	second, _ := pickSecondCard(memory, hidden, 0, true, nil, nil, nil, nil, nil)
...
func TestPickPair_PrefersElementMemoryWhenNoHighlight(t *testing.T) {
	...
	for range 20 {
		first, _, _ := pickPair(memory, hidden, true, nil, hiddenByElement, nil, nil)
		if first != 1 && first != 3 {
...
func TestPickSecondCard_PrefersSameElementFromElementMemory(t *testing.T) {
	...
	memory := map[int]int{}
	for range 20 {
		second, _ := pickSecondCard(memory, hidden, 3, true, nil, elementMemory, hiddenByElement, nil, nil)
		if second != 0 && second != 1 {
...
func TestPickPair_PrefersUnseenTilesWhenBestMove(t *testing.T) {
	// useBestMove=true: hidden 0,1,2,3; indices 0 and 1 have been revealed (knownIndicesSet), so unseen = 2,3.
	hidden := []int{0, 1, 2, 3}
	memory := map[int]int{}
	knownIndicesSet := map[int]struct{}{0: {}, 1: {}}
	for range 30 {
		first, _, reason := pickPair(memory, hidden, true, nil, nil, knownIndicesSet, nil)
		if first != 2 && first != 3 {
			t.Errorf("pickPair with useBestMove should prefer unseen tiles (2 or 3), got %d", first)
		}
		if reason != "unseen" {
			t.Errorf("pickPair with unseen tiles should return reason 'unseen', got %q", reason)
		}
	}
}

func TestPickSecondCard_PrefersUnseenTilesWhenBestMove(t *testing.T) {
	// useBestMove=true: firstIdx=0 (we flipped it), candidates = 1,2,3. Indices 0 and 1 have been revealed (knownIndicesSet), so unseen = 2,3.
	hidden := []int{1, 2, 3}
	memory := map[int]int{}
	knownIndicesSet := map[int]struct{}{0: {}, 1: {}}
	for range 30 {
		second, reason := pickSecondCard(memory, hidden, 0, true, nil, nil, nil, knownIndicesSet, nil)
		if second != 2 && second != 3 {
			t.Errorf("pickSecondCard with useBestMove should prefer unseen tiles (2 or 3), got %d", second)
		}
		if reason != "unseen" {
			t.Errorf("pickSecondCard with unseen tiles should return reason 'unseen', got %q", reason)
		}
	}
}

func TestPickPair_ReturnsRandomWhenNotUseBestMove(t *testing.T) {
	hidden := []int{0, 1, 2, 3}
	memory := map[int]int{0: 5, 1: 5} // known pair at 0,1
	hiddenSet := make(map[int]struct{})
	for _, idx := range hidden {
		hiddenSet[idx] = struct{}{}
	}
	for range 30 {
		first, _, reason := pickPair(memory, hidden, false, nil, nil, nil, nil)
		if reason != "random" {
			t.Errorf("pickPair with useBestMove=false should return reason 'random', got %q", reason)
		}
		if _, ok := hiddenSet[first]; !ok {
			t.Errorf("pickPair with useBestMove=false should return a hidden index, got %d", first)
		}
	}
}

func TestPickSecondCard_ReturnsRandomWhenNotUseBestMove(t *testing.T) {
	// No known pair (memory[0] not set or pair mate not in hidden), so we hit the random branch
	hidden := []int{1, 2, 3}
	memory := map[int]int{} // no pair known for firstIdx=0
	candidatesSet := map[int]struct{}{1: {}, 2: {}, 3: {}}
	for range 30 {
		second, reason := pickSecondCard(memory, hidden, 0, false, nil, nil, nil, nil, nil)
		if reason != "random" {
			t.Errorf("pickSecondCard with useBestMove=false should return reason 'random', got %q", reason)
		}
		if _, ok := candidatesSet[second]; !ok {
			t.Errorf("pickSecondCard with useBestMove=false should return a candidate index, got %d", second)
		}
	}
}
